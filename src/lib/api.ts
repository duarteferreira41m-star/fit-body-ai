...
export const api = {
  ...
  adjustDiet: (payload: DietAdjustmentRequest) => {
    const form = new FormData();
    form.append("message", payload.message);
    if (payload.availableFoods) {
      form.append("availableFoods", payload.availableFoods);
    }
    form.append("targetMacros", JSON.stringify(payload.targetMacros));
    if (payload.photo) {
      form.append("photo", payload.photo);
    }
    return apiFetch<{ adjustment: DietAdjustment }>("/api/diet/adjust", {
      method: "POST",
      body: form,
      auth: true,
    });
...
export type DietAdjustmentItem = {
  name: string;
  quantity: string;
  calories: number;
  proteinG: number;
...
export type DietAdjustmentRequest = {
  message: string;
